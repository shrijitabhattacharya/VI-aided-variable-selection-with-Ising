...
def gibbs_gamma(gamma,i,eta,mu,XXI,Xy,XXJI,B,a0,b0,n,m,rho):
    ...
    V3=b0*torch.sum(B[i,:]*gamma)+a0[i]
...
X= read_data('X_NeurIPS_indep_rp20_cp50_n100.txt') 
X= torch.tensor(X)
Ys= read_data('Y_NeurIPS_indep_beta84_rp20_cp50_n100.txt')
...
gamma_ini=2*torch.bernoulli(0.5*torch.ones((p,)))-1 # 0.5 0.1
...
yy=torch.sum(y*y)
mu_ini=param_init(y,X, 'lasso', alpha=0.01) # alpha 0.01 0.1
...
# torch.save(post_prob_sum, 'post_prob_sum_VB_beta84_indepX_rp20_cp50_n300_w7_tau03_lr1e-3_alp001.pt') 
